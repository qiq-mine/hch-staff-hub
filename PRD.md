...
- [三、 总体产品架构与业务流转](#三-总体产品架构与业务流转)
  ...
  - [3.1 企业级数字员工核心体系架构图](#31-企业级数字员工核心体系架构图)
  - [3.2 产品逻辑分层架构图](#32-产品逻辑分层架构图)
  - [3.3 典型端到端业务流转图](#33-典型端到端业务流转图)
...
> **图解说明**：数字员工架构由五维能力深度聚合而成：
> - **全渠道接入网关（左翼）**：飞书、企微、微信、钉钉等企业触点，提供意图智能分发与会话粘性保护；
> - **认知与规划大脑（顶核）**：大模型推理中枢结合 `TurnPlanner` 任务帧分解与长期情境记忆库；
> - **业务确定性骨骼（中枢）**：有限状态机（FSM）SOP 运行时，严格限定步骤、槽位与操作边界，保障交付质量；
> - **物理执行双手（底座）**：Harness 安全沙箱工作区与 MCP / 企业系统 API 连接器，实现真实业务操作；
> - **OKF 结构感知知识库（右翼）**：文档目录树两阶段定向检索与原子引证，杜绝模型幻觉；
> - **多 Agent 协同与自进化闭环（底翼）**：TL 领导 + 3 轮血条竞标赛制 + 团队共享黑板，结合全链路 Trace 与 PR 提案自演进。
